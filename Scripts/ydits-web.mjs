...
export class YditsWeb {
    ...
    mainloop() {
        ...
        if (dateNow - this.cnt_getNTP >= 1000) {
            this.cnt_getNTP = dateNow;
            getNtp();
            clock();
        }
    ...
    getNtp() {
        axios.head(window.location.href, { headers: { 'Cache-Control': 'no-cache' } })
            .then(res => {
                this.gmt = new Date(res.headers.date);

                timeYear = this.gmt.getFullYear();
                timeMonth = this.gmt.getMonth() + 1;
                timeDay = this.gmt.getDate();
                timeHour = this.gmt.getHours();
                timeMinute = this.gmt.getMinutes();
                timeSecond = this.gmt.getSeconds();
            }).catch(error => {
    ...
    makeDate_yahooeew() {
        let yahooeewDate = new Date(this.gmt);
